...
def run_diagnostic_batch(steps_to_run=500):
    ...
    with open(log_file_path, "w", encoding="utf-8") as f:
        ...
        for i in range(steps_to_run):
            ...
            if (
                ...
                or p_short > 0.5
                or current_pos != 0
                or abs(dir_action) > 0.6
            ):
                f.write(f"--- STEP {step_num} ---\n")
                f.write(
                    f"📊 ORACLE PROBS  | Hold: {p_hold:.2f} | Long: {p_long:.2f} | "
                    f"Short: {p_short:.2f}\n"
                )
                f.write(
                    f"🤖 MANAGER ACTION| Direction Out: {dir_action:.3f} | "
                    f"TP Mult Out: {tp_action:.3f}\n"
                )

                if current_pos != 0:
                    pos_str = "LONG" if current_pos == 1 else "SHORT"
                    f.write(
                        f"📈 ACTIVE TRADE  | Type: {pos_str} | Equity: ${equity:.2f}\n"
                    )
                else:
    ...
    print(
        f"Diagnostic complete! Check '{log_file_path}' to see exactly what the bot was thinking."
    )
