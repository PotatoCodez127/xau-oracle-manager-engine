...
class TradingSequenceDataset(Dataset):
    ...
    def __init__(self, csv_path: str, window_size: int = 30, scaler_path: str = './oracle_scaler.npz'):
        ...
        # We save this exact state so the live trading engine normalizes new market data identically.
...
def train_oracle(csv_path: str, scaler_path: str = './oracle_scaler.npz', epochs: int = 20, batch_size: int = 128, lr: float = 0.001):
    dataset = TradingSequenceDataset(csv_path, window_size=30, scaler_path=scaler_path)
    
    # Check if dataset is large enough to split
    if len(dataset) < 10:
        print("Dataset too small for train/val split. Using entire dataset for training (Test Mode).")
    ...
    # Weighted Cross Entropy Loss penalizes the network heavily if it misses rare high-probability setups
