...
class HybridTradingEnv(gym.Env):
    ...
    def __init__(
        ...
        if os.path.exists(scaler_path):
            ...
            raise FileNotFoundError(
                ...
                "Calculated parameters must be derived exclusively from the active "
                "training partition "
        ...
        if os.path.exists(oracle_path):
            self.oracle.load_state_dict(
                torch.load(oracle_path, map_location=self.device, weights_only=True)
            )
        self.oracle.eval()
        ...
        self.observation_space = spaces.Box(
            low=-100.0, high=100.0, shape=(9,), dtype=np.float32
        )
    ...
    def _next_observation(self):
        ...
        with torch.no_grad():
            tensor_obs = (
                torch.tensor(norm_obs, dtype=torch.float32).unsqueeze(0).to(self.device)
            )
    ...
    def step(self, action):
        ...
        if self.position != 0:
            ...
            if not trade_closed_this_step and self.bars_held >= self.max_hold:
                pnl = (
                    (current_price - self.entry_price) * self.position_size
                    if self.position == 1
                    else (self.entry_price - current_price) * self.position_size
                )
                reward += pnl
                self.balance += pnl
                trade_closed_this_step = True
